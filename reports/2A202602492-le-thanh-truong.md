...
- Nhóm: K4 — L3B
- Repository/branch: `K4-L3B-RAG-Pipeline` — nhánh `truong`

## Phần việc đã thực hiện

| Module/deliverable | Việc tôi trực tiếp làm | File/commit/PR | Trạng thái |
|---|---|---|---|
| **Task 5** — Semantic search | Embed query bằng đúng `embed_texts()` của Task 4; truy vấn ChromaDB; đổi cosine distance thành similarity; trả `SearchResult` đã sort giảm dần và không vượt `top_k` | `src/task5_semantic_search.py` | Done |
| **Task 6** — Lexical search (BM25) | Tokenizer cho tiếng Việt; dựng BM25 trên cùng corpus chunk với Task 5; lọc theo token trùng với query thay vì theo score | `src/task6_lexical_search.py` | Done |
| **Task 7** — RRF fusion | Gộp nhiều bảng xếp hạng theo thứ hạng `sum(1/(k+rank))`, khử trùng theo `id`, đánh dấu `retrieval_method="hybrid"` | `src/task7_reranking.py` | Done |
...
1. **Quyết định:** Ở Task 6, lọc kết quả theo **token trùng với query** thay vì lọc theo `score > 0` như gợi ý trong stub.
   ...
   **Trade-off:** Phải tokenize corpus một lần nữa ở bước lọc (đã gộp chung một lần tokenize để không lặp), và kết quả phụ thuộc chất lượng tokenizer thay vì để BM25 tự quyết.
...
2. **Quyết định:** Task 6 tái tạo corpus bằng `chunk_documents(load_documents())` của Task 4 thay vì lưu một bản corpus riêng.
   ...
   **Trade-off:** Mỗi lần gọi `lexical_search` phải dựng lại BM25 index; chấp nhận được với corpus 330 chunk nhưng sẽ thành điểm nghẽn nếu corpus lớn hơn nhiều.

## Kiểm thử và kết quả

- `pytest tests/test_contracts.py` — **15/15 pass**, gồm 3 test đúng phần tôi làm:

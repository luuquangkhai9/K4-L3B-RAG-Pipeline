...
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "data" / "landing" / "news"
...
async def crawl_and_format():
    ...
    try:
        ...
        html2text = None
    h = html2text.HTML2Text() if html2text else None
    if h:
    ...
    async with async_playwright() as p:
        ...
        for idx, url in enumerate(ARTICLE_URLS, 1):
            ...
            try:
                ...
                markdown_text = (
                    h.handle(str(main_content)).strip()
                    if h else main_content.get_text("\n", strip=True)
                )
